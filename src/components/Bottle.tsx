...
// const WAVE_TILE_COUNT = 3;
const WAVE_OFFSET_Y = 60;
// We make the wave paths very tall so they fill the bottle to the bottom
        ...
        {hasWater && (
          <g transform={`translate(0 ${waveBaseY})`}>
            ...
            <g opacity="0.3">
              ...
              />
              {[0, 1, 2].map((i) => (
                <path key={`rear-${i}`} d={REAR_WAVE_PATH} transform={`translate(${i * WAVE_TILE_WIDTH} 0)`} fill="rgba(40, 150, 255, 1)" />
              ))}
            </g>

            {/* 2. Middle Wave - More aggressive parallax */}
            <g opacity="0.5">
              <animateTransform
                attributeName="transform"
                type="translate"
              ...
              />
              {[0, 1, 2].map((i) => (
                <path key={`mid-${i}`} d={MID_WAVE_PATH} transform={`translate(${i * WAVE_TILE_WIDTH} 0)`} fill="rgba(120, 210, 255, 0.6)" />
              ))}
            </g>

            {/* 3. Front Wave - High speed slosh */}
            <g opacity="0.85">
              <animateTransform
                attributeName="transform"
                type="translate"
              ...
              />
              {[0, 1, 2].map((i) => (
                <path key={`front-${i}`} d={FRONT_WAVE_PATH} transform={`translate(${i * WAVE_TILE_WIDTH} 0)`} fill="rgba(84, 191, 255, 0.95)" />
              ))}
            </g>
          </g>
        )}
